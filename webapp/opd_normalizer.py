...
def parse_cash_register(path: str) -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        col1 = row.iloc[1] if len(row) > 1 else None  # trans date
        ...
        # A data row has a numeric serial number in col 0
        ...
        # Determine FY from remarks date if present, else from trans date
...
def export_to_excel(
    ...
    Sheet 2 — cash_register : one row per cash entry (Bill Amount excluded)
        Columns: Transaction Date | Transaction No | Case No |
    ...
    ws2.append([
        "Transaction Date", "Transaction No", "Case No",
        "Patient Name", "Entry By", "Income", "Discount",
    ])
    ...
    for i, r in enumerate(cash_records, start=2):
        ...
        for col_idx, cell in enumerate(ws2[i], start=1):
            cell.fill = fill
            cell.font = norm_font
            cell.alignment = center_al if col_idx in (1, 2, 3, 6, 7) else left_al
