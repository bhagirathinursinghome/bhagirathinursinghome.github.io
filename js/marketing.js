  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      try {
        ...
        locText.textContent = '📍 ' + ((data.display_name ? data.display_name.substring(0, 60) : 'Location found')) + '...';
      } catch {
...
async function loadReport() {
  ...
  document.getElementById('reportSummary').innerHTML = `
    ...
    <div class="summary-box"><h3>${new Set(data.map(function(d) { return d.created_at ? d.created_at.split('T')[0] : ''; })).size}</h3><p>Days Active</p></div>
  `;
    ...
    <div class="data-card">
      ...
      <div class="data-card-body">
        ...
        <div style="grid-column:1/-1;">
          ...
          <span class="value">${v.location_name || ((v.latitude ? v.latitude.toFixed(4) : '?') + ', ' + (v.longitude ? v.longitude.toFixed(4) : '?'))}</span>
        </div>
        ${v.remark ? `
        <div style="grid-column:1/-1;">
